...
LRESULT CWinCalPosApp::OnAnswerCalPositionMOD(WPARAM wParam, LPARAM lParam)
{
	...
	else if (((output->BD2Pos >> 12) & 0x01) && ((output->BD2Pos >> 13) & 0x01))//BsB3A
	{
		mode = 28;
	}
...
LRESULT CWinCalPosApp::OnAnswerCalPositionRAN(WPARAM wParam, LPARAM lParam)//伪距、载波、多普勒信息
{
	...
	if(m_dTotalSeconds != totalSec )//&& m_dTotalSeconds > SECONDS_IN_WEEK
	{
...
void CWinCalPosApp::AddEphRadio(BD2_EPH* eph)
{
	
}
void CWinCalPosApp::AddIonRadio(BD2_ION* ion)
{
	...
	for (j=0; j<21; j++)
	{
		...
		for (i=0; i<ImportData[style].numuse; i++)
		{
			...
			else if (posmod<26)//B1CB2B3C
			{
				...
				epoch->SatInfo[SatNum].Pseudorang = - temp0*1.843645083933 + (ImportData[2].ran[SInd[2][SatID-1]].x+Diff31C)*2.843645083933;
				epoch->SatInfo[SatNum].Pseudorang += (-(Diff32C+ImportData[6].ran[SInd[6][SatID-1]].x)*7.524462924463+ temp0*8.524462924463);
			...
			else if (posmod <27)//B1CB2a
			{
				...
				if(!ObsCheck(ImportData[4].ran[SInd[4][SatID-1]].x))
					continue;
				temp1=ImportData[4].ran[SInd[4][SatID-1]].x-ImportEPH[SatID-1].tgd2*299792458;
			...
			else if (posmod<29)//BsB3AE
			{
				...
				if((Fs3E)&&(fabs( temp0-ImportData[11].ran[SInd[11][SatID-1]].x)<Them))
				{
					...
					if(i==ImportData[style].numuse)
						Fs3E=0;
					Diffs3E= Diffs3A*(k-1)+(temp0-ImportData[11].ran[SInd[11][SatID-1]].x-delta);
					Diffs3E/=k;
				}
				epoch->SatInfo[SatNum].Pseudorang = - temp0*0.349733060146 + (ImportData[11].ran[SInd[11][SatID-1]].x+Diffs3A)*1.349733060146;
				if(!ObsCheck(epoch->SatInfo[SatNum].Pseudorang))
