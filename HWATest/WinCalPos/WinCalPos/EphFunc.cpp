...
bool CalculateBD1SatPos(BD2_EPH eph,double Sec, BD2_SAT_POS* pResult)
{
	...
	Mdot = n0+eph.deltaN*PI;
	M = eph.m0*PI+Mdot*deltaToe;
	E = eph.e;
	...
	Edot = Mdot/(1.0-E*CE);
	Phi =atan2(sqrt1me2*SE,CE-E)+eph.w*PI;
	...
	I = eph.i0*PI+eph.idot*PI*deltaToe+(eph.cis*S2P+eph.cic*C2P);
	SI = sin(I);
	CI = cos(I);
	Idot = eph.idot*PI +2.0*Pdot*(eph.cis*C2P - eph.cic*S2P);
	Xpos = R*CU;
	Ypos = R*SU;
	...
	if(eph.id > 5)
	{
		L =eph.omega0*PI+deltaToe*(eph.omegadot*PI-7.292115E-5)-eph.toe*7.292115E-5;
		Ldot = eph.omegadot*PI-7.292115E-5; 
		SL =sin(L);
		CL =cos(L);
	...
	{
		L =eph.omega0*PI+deltaToe*eph.omegadot*PI-eph.toe*7.292115E-5;
		Ldot = eph.omegadot*PI; 
		SL =sin(L);
		CL =cos(L);
...
bool CalculateBD2SatPos(BD2_EPH eph,double Sec, BD2_SAT_POS* pResult)
{
	...
	deltaNA = eph.deltaN*PI + (eph.Ndot*PI * deltaToe)/2;
	NA = n0 + deltaNA;
	M = eph.m0*PI + NA*deltaToe;
	E = eph.e;// 轨道偏心率
	sqrt1me2 = sqrt(1-E*E);
	EI = M;
	for(k=0;k<15;k++)
	...
	SI = sin(I);
	CI = cos(I);
	Idot = eph.idot*PI + 2*Pdot*(eph.cis * C2P - eph.cic * S2P);
	Xpos = R*CU;
	Ypos = R*SU;
	Xdot = Rdot*CU+R*CUdot;
	Ydot = Rdot*SU+R*SUdot;
	OmegaDot = eph.omegadot*PI - 8.16814089933346242e-9;
	// 计算卫星坐标和速度
	if(eph.id > 5)
	{
		L =eph.omega0*PI+deltaToe*(OmegaDot-7.292115e-5)-eph.toe*7.292115e-5;
	...
	{
		L =eph.omega0*PI+deltaToe*OmegaDot-eph.toe*7.292115e-5;
		Ldot = OmegaDot; 
		SL =sin(L);
		CL =cos(L);
		BDx = Xpos*CL-Ypos*CI*SL;
