...
CDisposeNMEA::CDisposeNMEA(void)
{
	Reset();
...
int CDisposeNMEA::FindHead(char* buf, int len)
{
...
void CDisposeNMEA::DisposeData(char* buf, int len)
{
	char ch = 0;
	u4 crc = 0;
	...
	for (int i = 0; i < len; i++)
	{
		...
		case RECV_CMD_DATA:
			...
			{
				//crc = CheckCrc32(INIT_CRC, (u1*)(m_chBuffer+1), m_chLen-12);
				//if(crc == GetData_8CharToHex((char*)m_chBuffer+(m_chLen - 10)))
...
void CDisposeNMEA::DisposeIndex(int index)
{
	switch (index)
	{
	case OUT_GNREF:
...
/****************************************************************************************/
void CDisposeNMEA::DisposeSETMOD(u1* buf, int len)
{

}
void CDisposeNMEA::DisposeGNREF(char* buf, int len)
{
	int branch = 0;
	SETREF result;
	char chTemp[20][30];
...
void CDisposeNMEA::DisposeGNMOD(char* buf, int len)
{
	int branch = 0;
	SETMOD result;
	char chTemp[10][20];
...
void CDisposeNMEA::DisposeGNCOM(char* buf, int len)
{
	int branch = 0;
	SETCOM result;
	char chTemp[20][20];
...
void CDisposeNMEA::DisposeGNRAN(char* buf, int len)
{
	BD2_RAN result;
	char chTemp[200][30];
	char buffer[30];
...
void CDisposeNMEA::DisposeGNPOS(char* buf, int len)
{
	int branch = 0;
...
void CDisposeNMEA::DisposeGNXPV(char* buf, int len)
{
	int branch = 0;
	unsigned short id=0,index=0;
	BD2_POS result;
...
void CDisposeNMEA::DisposeGNTIM(char* buf, int len)
{
	int branch = 0;
	BD2_TIME result;
	char chTemp[15][30];
...
void CDisposeNMEA::DisposeGNSTA(char* buf, int len)
{
	int branch = 0;
	BD2_STA result;
	char chTemp[10][20];
...
void CDisposeNMEA::DisposeGNGSV(char* buf, int len)
{
...
void CDisposeNMEA::DisposeGNGBS(char* buf, int len)
{
	int branch = 0;
	BD2_GBS result;
	char chTemp[20][20];
...
void CDisposeNMEA::DisposeBDCHD(char* buf, int len)
{
	int branch = 0;
	BD2_GBS result;
	char chTemp[20][20];
...
void CDisposeNMEA::DisposeBDPAO(char* buf, int len)
{
	int branch = 0;
	BD2_STA result;
	char chTemp[10][20];
...
//一期
void CDisposeNMEA::DisposeBDEPH(char* buf, int len)
{
	int branch = 0;
	BD2_EPH result;
	char chTemp[30][30];
...
void CDisposeNMEA::DisposeBDALM(char* buf, int len)
{
	int branch = 0;
	BD2_ALM result;
	char chTemp[20][30];
...
void CDisposeNMEA::DisposeBDJAL(char* buf, int len)
{
	int branch = 0;
	BD2_JAL result;
	char chTemp[10][20];
...
void CDisposeNMEA::DisposeBDION(char* buf, int len)
{
	int branch = 0;
	BD2_ION result;
	char chTemp[20][20];
...
void CDisposeNMEA::DisposeGNCOD(char* buf, int len)
{
	int branch = 0;
	BD2_COD result;
	char chTemp[10][250];
...
//二期
void CDisposeNMEA::DisposeBSEPH(char* buf, int len)
{
	int branch = 0;
	BD2_EPH result;
	char chTemp[35][20];
...
void CDisposeNMEA::DisposeBSALM(char* buf, int len)
{
	int branch = 0;
	BD2_ALM result;
	char chTemp[20][20];
...
void CDisposeNMEA::DisposeBSJAL(char* buf, int len)
{
	int branch = 0;
	BD2_JAL result;
	char chTemp[10][20];
...
void CDisposeNMEA::DisposeBSION(char* buf, int len)
{
	int branch = 0;
	BD2_ION result;
	char chTemp[20][20];
...
/***********************************************************************************/
int CDisposeNMEA::PacketACKData(char* pBuff, const SETACK& data)//查询模式
{
	return 0;
}
int CDisposeNMEA::PacketPILOTData(char* pBuff, const SETPILOT& data)//数据导频
{
	return 0;
}
int CDisposeNMEA::PacketIONData(char* pBuff, const SETION& data)
{
	return 0;
}
int CDisposeNMEA::PacketBD2POSData(char* pBuff, const BD2_POS& data)
{
	return 0;
}
int CDisposeNMEA::PacketTIMData(char* pBuff, const SETTIM& data)
{
	u4 crc=0;
	int len = 0;
	char chBuff[50];
...
int CDisposeNMEA::PacketTMDData(char* pBuff, const SETTMD& data)
{
	u4 crc=0,branch=0;
	int len = 0, type=0;
	char chBuff[100];
...
int CDisposeNMEA::PacketCHAData(char* pBuff, const SETCHA& data)
{
	u4 crc=0;
	int len = 0;
	char chBuff[50];
...
int CDisposeNMEA::PacketCOMData(char* pBuff, const SETCOM& data)
{
	u4 crc=0;
	int len = 0;
	char chBuff[50];
...
int CDisposeNMEA::PacketMODData(char* pBuff, const SETMOD& data)
{
	u4 crc=0;
	int len = 0;
	char chBuff[100];
...
int CDisposeNMEA::PacketWORKMODData(char* pBuff, const WORKMOD& data)//add by cy 20180714
{
	u4 crc=0;
	int len = 0;
	char chBuff[100];
...
int CDisposeNMEA::PacketISTData(char* pBuff, const SETIST& data)//add by cy 20180714
{
	u4 crc=0;
	int len = 0;
	char chBuff[100];
...
int CDisposeNMEA::PacketLOGData(char* pBuff, const SETLOG& data)
{
	u4 crc=0;
	int len = 0;
	char chBuff[50];
...
int CDisposeNMEA::PacketPOSData(char* pBuff, const SETPOS& data)
{
	u4 crc=0;
	int len = 0;
	char chBuff[100];
...
int CDisposeNMEA::PacketWIDData(char* pBuff, const SETWID& data)
{
	u4 crc=0;
	int len = 0;
	char chBuff[50];
...
int CDisposeNMEA::PacketBDIData(char* pBuff, const SETREF& data)
{
	u4 crc=0;
	int len = 0;
	char chBuff[500];
...
int CDisposeNMEA::PacketREFData(char* pBuff, const SETREF& data)
{
	u4 crc=0;
	int len = 0;
	char chBuff[500];
...
int CDisposeNMEA::PacketMUTData(char* pBuff, const SETMUT& data)
{
	u4 crc=0;
	int len = 0;
	char chBuff[50];
...
int CDisposeNMEA::PacketEACData(char* pBuff, const SETEAC& data)
{
	u4 crc=0;
	int len = 0;
	char chBuff[50];
...
int CDisposeNMEA::PacketBGTData(char* pBuff, const SETBGT& data)
{
	u4 crc=0;
	int len = 0;
	char chBuff[200];
...
int CDisposeNMEA::PacketTTFData(char* pBuff, const SETTTF& data)
{
	u4 crc=0;
	int len = 0;
	char chBuff[50];
...
int CDisposeNMEA::PacketSTAData(char* pBuff, const SETSTA& data)
{
	u4 crc=0;
	int len = 0;
	char chBuff[50];
...
int CDisposeNMEA::PacketSTSData(char* pBuff, const SETSTS& data)
{
	u4 crc=0;
	int len = 0;
	char chBuff[50];
...
int CDisposeNMEA::PacketEVAData(char* pBuff, const SETEVA& data)
{
	u4 crc=0;
	int len = 0;
	char chBuff[50];
...
int CDisposeNMEA::PacketACQData(char* pBuff, const SETACQ& data)//捕获模式
{
	u4 crc = 0;
	int len = 0;
...
int CDisposeNMEA::PacketTRKData(char* pBuff, const SETTRK& data)//跟踪模式
{
	u4 crc = 0;
	int len = 0;
	return len;
...
int CDisposeNMEA::PacketEPQData(char* pBuff, const SETEPQ& data)
{
	u4 crc=0;
	int len = 0;
	char chBuff[80];
...
//private//
void CDisposeNMEA::AnalyzeNmeaData(char *start_buffer, int length)
{
	int i,j,k, t=0;
	int GSV_num=0;
	char bb[30][12];
	...
	else if(*(start_buffer+3) == 'G' && *(start_buffer+4) == 'G' && *(start_buffer+5) == 'A')
	{
		...
		memset((void *)&save_GGA, 0x0, sizeof(struct nmea_GGA));
...
//save to nmea struct
	...
	else if(nmea_char_save.cmd == GSV)	//ID\CN0\view
	{
		...
		if(nmea_char_save.sys == GP)
		{
			...
			{
				int n = nmea_char_save.gp_n;//使用卫星数
...
int CDisposeNMEA::CalCheckVal(char *pData, int len)
{
...
int CDisposeNMEA::CheckData(char *start)
{
    int nmea_cr = 0;
    int t = 0;
...
void CDisposeNMEA::strToTime(int time_clk)
{
	//日月年
...
void CDisposeNMEA::strToTime1(int time_date)
{
	//时分秒
	nmea_char_save.UTC.hour = time_date/10000;
	nmea_char_save.UTC.minute = time_date/100%100;
	nmea_char_save.UTC.second = time_date%100;
}
